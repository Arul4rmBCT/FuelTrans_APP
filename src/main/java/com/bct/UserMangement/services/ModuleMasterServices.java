package com.bct.UserMangement.services;

import java.util.List;

import javax.servlet.ServletContext;
import javax.ws.rs.Consumes;
import javax.ws.rs.POST;
import javax.ws.rs.Path;
import javax.ws.rs.core.Context;
import javax.ws.rs.core.HttpHeaders;
import javax.ws.rs.core.Response;
import javax.ws.rs.core.UriInfo;

import com.bct.UserMangement.bo.ModuleMaster;
import com.bct.UserMangement.bo.ResponseBO;
import com.bct.UserMangement.dao.ModuleMasterDAO;
import com.bct.UserMangement.utils.Errors;
import com.bct.UserMangement.utils.RequestLogger;
import com.bct.UserMangement.utils.RestUtils;
import com.bct.UserMangement.utils.UserMangementException;
import com.google.gson.Gson;

import net.sf.json.JSONArray;

@Path("/v1")
public class ModuleMasterServices {
	
	ModuleMasterDAO dao = new ModuleMasterDAO();
	Gson gson = new Gson();
	
	@POST
	@Consumes({ "application/json" })
	@Path("/module")
	@RequestLogger
	public Response userOperation(@Context HttpHeaders headers, @Context UriInfo ui, @Context ServletContext context, String reqJsonString) {
		String token = context.getInitParameter("authKey");
		String htoken = null;
		
		ResponseBO responseObj = new ResponseBO();	
		JSONArray dataSet = new JSONArray();
		try {
			System.out.println(reqJsonString);
			if (headers.getRequestHeader("token") != null && headers.getRequestHeader("token").size() > 0)
				htoken = headers.getRequestHeader("token").get(0);
            System.out.println("htoken"+htoken); 
            System.out.println("token"+token); 
            
		    if(htoken == null)
		    	return RestUtils.constructError(Errors.INVALID_TOKEN);
		    ModuleMaster module= gson.fromJson(reqJsonString,ModuleMaster.class);
			if(module == null)
				return RestUtils.constructError(Errors.INVALID_INPUT);
			System.out.println("falg"+module.getFlag());
			System.out.println("user"+module.getModule_id());
			
			delegateOperations(module,dataSet,responseObj);
			
			String str = gson.toJson(responseObj);
			return RestUtils.getReponse(Response.Status.CREATED, str, "application/json");
			
		} catch (UserMangementException e) {
			e.printStackTrace();
			return RestUtils.constructError(e.getError());
		}
		
	}
	
	private void delegateOperations(ModuleMaster module, JSONArray dataSet, ResponseBO responseObj) throws UserMangementException {
		if(module.getFlag().equalsIgnoreCase("c")) {
			createModule(module,dataSet,responseObj);
		} else if(module.getFlag().equalsIgnoreCase("r")) {
			getModules(module.getModule_id(),dataSet,responseObj);
		} else if(module.getFlag().equalsIgnoreCase("u")) {
			dao.updateModule(module);
			responseObj.setErrorCode("1");
			responseObj.setError("");
			responseObj.setStatus("Success");
		}else if(module.getFlag().equalsIgnoreCase("d")) {
			dao.deleteModule(module.getModule_id());
			responseObj.setErrorCode("1");
			responseObj.setError("");
			responseObj.setStatus("Success");
		}
	}

	private void getModules(String module_id,JSONArray dataSet, ResponseBO responseObj) throws UserMangementException{
		if(module_id !=null && !module_id.isEmpty()) {
			ModuleMaster response = dao.getModule(module_id);
			if(response == null)
				throw new UserMangementException(Errors.MODULE_NOT_FOUND);
			dataSet.add(gson.toJson(response));
			responseObj.setDataArr(dataSet);
		}else {
			List<ModuleMaster> modules = dao.getModuleList();
			if(modules.isEmpty())
				throw new UserMangementException(Errors.MODULE_NOT_FOUND);
			dataSet.add(gson.toJson(modules));
			responseObj.setDataArr(dataSet);
     	}
		responseObj.setErrorCode("0000");
		responseObj.setError("");
		responseObj.setStatus("Success");
	}
	
	private void createModule(ModuleMaster module, JSONArray dataSet, ResponseBO responseObj) throws UserMangementException {
		String res = dao.createModule(module);
		if(res.isEmpty()) {
			dataSet.add(gson.toJson(res));
			responseObj.setDataArr(dataSet);
			responseObj.setErrorCode("0000");
			responseObj.setError("");
			responseObj.setStatus("Success");
		}else {
			if(res.contains("violates unique constraint")) { 
				throw new UserMangementException(Errors.ROLE_ORG_UNIQUE);
			} else {
				throw new UserMangementException(Errors.INVALID,res);	
			}
		}	
		
	}
		
}
